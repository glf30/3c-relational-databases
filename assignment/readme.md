...
- For getting a specific movie by its ID, you should utilize the model's [.findById()](https://mongoosejs.com/docs/api/model.html#Model.findById()) function.  You will need to come up with a way to return both the movie object as well as the array of reviews in a single function. If you are having trouble, first start by getting all of the reviews associated with the movie without worrying about the Movie data.
